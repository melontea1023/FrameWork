...
void Renderer::Init()
{
	...
	// デバイス、スワップチェーン作成
	//描画結果をディスプレイに表示するため、バックバッファの数やウィンドウサイズの設定
	...
	//関数を使ってDirectXデバイス、コンテキスト、スワップチェーンを作成
...
// Renderer.cpp に追加
void Renderer::SetCamera(int id) {
	// 今は 2D 用の固定カメラを想定（カメラID = 1 の場合）

	if (id == 1) {
		// 正射影プロジェクション（画面サイズベース）
		float width = static_cast<float>(Application::GetWidth());
		float height = static_cast<float>(Application::GetHeight());

		// 正射影（左上が (0,0)、右下が (width, height) のような扱い）
		DirectX::SimpleMath::Matrix view = DirectX::SimpleMath::Matrix::Identity;
		DirectX::SimpleMath::Matrix proj = DirectX::SimpleMath::Matrix::CreateOrthographicOffCenter(
			0.0f, width,     // Left, Right
			height, 0.0f,    // Bottom, Top（Y反転に注意）
			0.0f, 1.0f       // Near, Far
		);

		m_DeviceContext->UpdateSubresource(m_ViewBuffer, 0, nullptr, &view, 0, 0);
		m_DeviceContext->UpdateSubresource(m_ProjectionBuffer, 0, nullptr, &proj, 0, 0);
	}
	// TODO: 他のIDで3Dカメラなどを追加可能
}
